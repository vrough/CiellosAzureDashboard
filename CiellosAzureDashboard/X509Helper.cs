...
{
    public class X509Helper : IX509Helper
    {
        private const string password = "securePassword";
        private string applicationName { get; set; }
        private const string RootCAName = "CiellosDashboard";
        private const string RootCALocation = RootCAName + ".pfx";

        public X509Helper(IHostingEnvironment env)
        {
        ...
        internal void SaveCertificateToPfxFile(string filename, string password,
        ...
        internal X509Certificate2 ExportCertificatePublicKey(X509Certificate2 certificate)
        {
        ...
        internal X509Certificate2 CreateCaCertificate(string subjectName, string certificatePassword, X509Certificate2 issuingCa)
        {
            using (RSA rsa = RSA.Create(2048))
            {
                ...
                if (issuingCa != null)
                {
                    ...
                    return generatedCertificate.CopyWithPrivateKey(rsa);
                }
                else
                {
        ...
        internal (X509Certificate2 certificate, X509Certificate2Collection collection)LoadCertificateAndCollectionFromPfx(string pfxFileName, string password)
        {
        ...
        internal X509Certificate2 CreateAndSignCertificate(string subjectName, X509Certificate2 signingCertificate)
        {
        ...
        {
            var rootCaCert = CreateCaCertificate(RootCAName, password, null);
            SaveCertificateToPfxFile($"{RootCAName}.pfx", password, CreateAndSignCertificate(RootCAName, rootCaCert), null, null);
        }
        ...
        public void RotateCertificate()
        {
            if (!ifRootCaExist())
            {
                CreateAndSaveRootCertificate();
            }
            if (ifRootCaExpired())
            {
                File.Delete(RootCALocation);
                CreateAndSaveRootCertificate();
            }
            //var (caCert, caCertCollection) = LoadCertificateAndCollectionFromPfx(RootCAName, password);
         //   var deviceCert = CreateAndSignCertificate(applicationName, caCert);
           // var publicKeyBytes = deviceCert.Export(X509ContentType.Cert);
           // File.WriteAllBytes($"{applicationName}.cer", publicKeyBytes);
        }
        public X509Certificate2 GetRootCertificate()
        {
            var (caCert, caCertCollection) = LoadCertificateAndCollectionFromPfx(RootCALocation, password);
            return caCert;
        }
        public DateTime GetRootCAExpirationDate()
        {
            return GetRootCertificate().NotAfter;
            
        }
        private bool ifRootCaExpired()
        {
            bool ret = false;
            if (ifRootCaExist())
            {
                ret = GetRootCertificate().NotAfter < DateTime.UtcNow;
            }
            return ret;
        }
        private bool ifRootCaExist()
        {
            return File.Exists(RootCALocation);
        }
        public X509Certificate2 GetPublicKeyCertificate()
        {
            if (ifRootCaExpired()) RotateCertificate();
            var rootPublicKey = ExportCertificatePublicKey(GetRootCertificate());
            var rootPublicKeyBytes = rootPublicKey.Export(X509ContentType.Cert);
            return new X509Certificate2(rootPublicKeyBytes);
            //File.WriteAllBytes($"{applicationName}.cer", rootPublicKeyBytes);
        }
    }
}